...
- (void)mapView:(MKMapView *)mapView regionWillChangeAnimated:(BOOL)animated
{
    ...
    [[ZTTGHttpTool sharedZTTGHttpTool] dealsWithLatitude:pos.latitude longitude:pos.longitude success:^(NSArray *deals, int totalCount) {
        for (ZTDeal *deal in deals) {
            // 已经显示过
            ...
            for (ZTBusiness *business in deal.businesses) {
                ZTDealAnnotation *anno = [[ZTDealAnnotation alloc] init];
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(ZTDealAnnotation *)annotation
{
    if (![annotation isKindOfClass:[ZTDealAnnotation class]]) return nil;
...
- (void)mapView:(MKMapView *)mapView didSelectAnnotationView:(MKAnnotationView *)view
{
    NSLog(@"您点击了大头针！");
    // 展示详情
    ZTDealAnnotation *anno = view.annotation;
...
- (void) buttonTapped:(UIButton *)sender
{
    ...
    if (self.currentDeal_id) {
        // 跳转到团购
        ZTDetailDealController *detailDealCtrl = [[ZTDetailDealController alloc]init];
...
- (void)dealloc
{
    ...
    NSLog(@"KLMapViewController dealloc");
}
