...
- (void)viewDidLoad {
    ...
    self.title = @"天气预报";
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithImage:@"ios7_top_navigation_back" target:self action:@selector(Canccel)];
...
- (void)setupSimapleWeathers
{
    ...
    CGFloat simapleX = 12;
    CGFloat simapleWeatherY = CGRectGetMaxY(self.detailLable.frame) + 20;
    if (is4Inch) {
        simapleWeatherY +=10;
    }
    CGFloat simapleWeatherH = kScreenHeight - 350;
    CGFloat simapleWeatherW = (kScreenWidth - 2 * simapleX)/3;
    
    NSMutableArray *simapleWeathers = [NSMutableArray array];
    for (int i = 0; i < 3; ++i) {
        ZTMoreWeather *simapleWeather = [ZTMoreWeather getMoreWeather];
...
- (void)setupWeatherInfo
{
    ...
    ZTIndexDetail *detail = self.weatherInfo.index[0];
    ...
    self.wetherImageView.image = [UIImage imageNamed:weather];
    ...
    for (int i = 1; i < 4; ++i) {
        ...
        ZTMoreWeather *moreView = self.simapleweathers[i-1];
        ...
        moreView.weatherImageView.image = [UIImage imageNamed:weather];
        
        moreView.dateLable.text = weatherData.date;
        moreView.weatherLable.text = weatherData.weather;
        moreView.windLable.text = weatherData.wind;
        moreView.temperatureLable.text = weatherData.temperature;
